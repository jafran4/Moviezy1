  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-5xl bg-white text-neutral-900 rounded-none sm:rounded-3xl overflow-hidden shadow-2xl my-auto border border-neutral-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button (TV Remote & Mouse) */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-30 p-2.5 rounded-full bg-white/90 hover:bg-white text-neutral-900 border border-neutral-200 shadow-md transition cursor-pointer focus:ring-4 focus:ring-amber-400 outline-none"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2 space-y-4">
              <div className="flex flex-wrap items-center gap-3 text-sm font-semibold">
                ...
                <span className="text-neutral-700 font-bold">{releaseYear}</span>
                <span className="px-2 py-0.5 border border-neutral-300 rounded text-xs text-neutral-800 font-bold">
                  {media.maturityRating || "16+"}
                </span>
                <span className="text-neutral-600">{media.duration || "2h 15m"}</span>
                <span className="px-2 py-0.5 border border-amber-400 text-amber-700 bg-amber-50 rounded text-xs font-black">
                  4K ULTRA HD
                </span>
                <span className="px-2 py-0.5 border border-neutral-300 rounded text-xs text-neutral-600 font-bold">
                  DOLBY ATMOS
                </span>
              </div>
              ...
              {onOpenStore && (
                <div className="p-4 bg-gradient-to-r from-amber-50 via-amber-100/60 to-orange-50 border border-amber-300 rounded-2xl flex items-center justify-between shadow-2xs">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-neutral-950 font-black text-xs sm:text-sm rounded-xl transition shadow-xs cursor-pointer focus:ring-4 focus:ring-amber-400 outline-none"
                  >
                    View Plans ($1.65)
                  </button>
                </div>
            ...
            <div className="space-y-4 text-xs sm:text-sm text-neutral-600">
              {credits.cast && credits.cast.length > 0 && (
                <div>
                  ...
                  <p className="text-neutral-800 line-clamp-3 leading-relaxed">
              ...
              {credits.director && (
                <div>
                  ...
                  <p className="text-neutral-800">{credits.director}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                {similar.slice(0, 8).map((item) => (
                  <div
                    ...
                    className="bg-neutral-50 rounded-xl overflow-hidden border border-neutral-200 hover:border-neutral-400 focus:border-amber-400 focus:ring-4 focus:ring-amber-400 focus:scale-105 transition cursor-pointer outline-none shadow-2xs"
                  >
                    <div className="relative aspect-video">
                      <img
                        ...
                        loading="lazy"
                      />
                      <div className="absolute top-2 right-2 px-1.5 py-0.5 bg-neutral-900/80 rounded text-[10px] font-bold text-white">
