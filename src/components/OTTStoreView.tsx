...
const OTTStoreView: React.FC<OTTStoreViewProps> = ({
  ...
  const [storeCurrency, setStoreCurrency] = useState<"USD" | "EUR" | "QAR">("USD");

  const formatStorePrice = (usdVal: number) => {
    if (storeCurrency === "EUR") {
      return `€${(usdVal * 0.92).toFixed(2)}`;
    }
    if (storeCurrency === "QAR") {
      return `${Math.round(usdVal * 3.64)} QAR`;
    }
    return `$${usdVal.toFixed(2)}`;
  };

  const formatStorePillPrice = (usdVal: number) => {
    if (storeCurrency === "EUR") {
      return `€${(usdVal * 0.92).toFixed(1)}`;
    }
    if (storeCurrency === "QAR") {
      return `${Math.round(usdVal * 3.64)} QAR`;
    }
  ...
  return (
    <div id="ott-store-main-view" className="w-full max-w-7xl mx-auto px-4 sm:px-8 md:px-12 pt-24 pb-20 animate-fadeIn">
      ...
      <div className="relative text-center mb-12 sm:mb-16 space-y-4">
        ...
        {/* Facebook Page Quick Buy Contact Banner */}
        <div className="max-w-4xl mx-auto mt-6 bg-[#1877F2]/15 border-2 border-[#1877F2]/50 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4 text-white shadow-xl backdrop-blur-sm">
          <div className="flex items-center space-x-3 text-center sm:text-left">
            <div className="w-10 h-10 rounded-xl bg-[#1877F2] text-white flex items-center justify-center flex-shrink-0 shadow-md">
              <Facebook className="w-6 h-6 fill-white text-[#1877F2]" />
            </div>
            <div>
              <h3 className="text-sm sm:text-base font-black text-white">
                Official Facebook Buying &amp; Support Channel
              </h3>
              <p className="text-xs text-blue-200 mt-0.5">
                Click below to contact our official Facebook page directly for custom packages, instant credentials, and 24/7 activation.
              </p>
            </div>
          </div>
          ...
          <a
            ...
            className="w-full sm:w-auto px-5 py-2.5 bg-[#1877F2] hover:bg-[#166fe5] text-white rounded-xl text-xs font-black transition flex items-center justify-center space-x-2 flex-shrink-0 shadow-lg hover:scale-105"
          >
            <Facebook className="w-4 h-4 fill-white text-[#1877F2]" />
            <span>Contact Us on Facebook to Buy</span>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
        <div className="flex items-center space-x-2 sm:space-x-3 overflow-x-auto pb-2 w-full sm:w-auto">
          {categories.map((cat) => (
            <button
              key={cat}
        ...
        {/* Currency Switcher */}
        <div className="flex items-center bg-neutral-900/90 p-1 rounded-xl border border-neutral-800 flex-shrink-0 shadow-md">
          <button
            type="button"
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                    {service.plans.map((p, idx) => (
                      ...
                      >
                        <div>{p.duration}</div>
                        <div className="text-[10px] opacity-90">{formatStorePillPrice(p.price)}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-neutral-900/90 border border-neutral-800/80 rounded-xl p-3.5 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <span className="text-xs text-neutral-500 line-through block">
                      {formatStorePrice(activePlan.originalPrice)}
                    </span>
                    <div className="text-2xl font-black text-white leading-none">
                      {formatStorePrice(activePlan.price)}
                    </div>
                ...
                <div className="space-y-2.5 pt-2">
                  ...
                  <div className="space-y-2">
                    <button
                      onClick={() => {
                        const convertedPlan = {
                          ...
                          price:
                            storeCurrency === "EUR"
                              ? Number((activePlan.price * 0.92).toFixed(2))
                              : storeCurrency === "QAR"
                              ? Math.round(activePlan.price * 3.64)
                              : activePlan.price,
                          originalPrice:
                            storeCurrency === "EUR"
                              ? Number((activePlan.originalPrice * 0.92).toFixed(2))
                              : storeCurrency === "QAR"
                              ? Math.round(activePlan.originalPrice * 3.64)
                              : activePlan.originalPrice,
                        };
                        onSelectPlanForCheckout(service, convertedPlan);
                      }}
                    ...
                    >
                      ...
                      <span>Buy via Facebook ({formatStorePrice(activePlan.price)})</span>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full py-2 bg-[#1877F2]/20 hover:bg-[#1877F2]/30 border border-[#1877F2]/40 text-white rounded-xl text-xs font-bold transition flex items-center justify-center space-x-1.5 cursor-pointer"
                    >
                      <Facebook className="w-3.5 h-3.5 fill-white text-[#1877F2]" />
                      <span>Contact on Facebook to Buy</span>
                    </a>
                  </div>
                </div>
              </div>
